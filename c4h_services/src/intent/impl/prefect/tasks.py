...
def run_agent_task(
    ...
    try:
        ...
        try:
            ...
            prefect_logger.debug(f"Lineage config retrieved: {lineage_config}")
            
            if lineage_config and lineage_config.get("enabled", True):
                prefect_logger.debug(f"Lineage is enabled, initializing EventLogger with run_id: {run_id}")
                try:
                    ...
                    prefect_logger.info(f"Initialized event logger for agent: {task_config_copy.get('name')}, run_id: {run_id}")
                except Exception as e:
                    prefect_logger.error(f"Failed to initialize event logger: {str(e)}", 
                                       exc_info=True)
            else:
                prefect_logger.debug("Lineage not enabled or config empty", 
                                   enabled=lineage_config.get("enabled") if lineage_config else None,
                                   config_exists=bool(lineage_config))
        except Exception as e:
            prefect_logger.error(f"Failed to get lineage config: {str(e)}", 
                               exc_info=True)
        ...
        # Accept any dict-like mapping object (dict, DictConfig, etc.)
        if execution_plan is None or not isinstance(execution_plan, Mapping) or not execution_plan.get("enabled", True):
        ...
        try:
            ...
            # IMPORTANT: Do NOT return the entire context to avoid data accumulation
            # Only return the output data that was specifically generated by this execution
            response = {
                "success": execution_result.success,
                "result_data": execution_result.output or {},
                # Don't include full context - it contains accumulated data from previous teams
                # "context": execution_result.context,  # REMOVED to prevent data leakage
