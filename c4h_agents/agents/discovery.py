"""
Discovery agent stub for backward compatibility.
Path: c4h_agents/agents/discovery.py
"""

from typing import Dict, Any
from c4h_agents.agents.generic import GenericLLMAgent

class DiscoveryAgent(GenericLLMAgent):
    """
    Legacy discovery agent, now implemented using GenericLLMAgent.
    ...
    def __init__(self, full_effective_config: Dict[str, Any], unique_name: str):
        ...
        # Set persona to discovery if not already set
        if not hasattr(self, 'persona_key') or not self.persona_key:
            self.persona_key = "discovery"
        
        # Log that we're using the legacy class
        self.logger.info("legacy.discovery_agent.initialized", 
